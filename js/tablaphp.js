/*!
 * tabla.js v1.1 por Departamento de Informatica
 * 2013 Universidad Galileo
 *

 :Contains sin distinguir entre mayusculas y minusculas

JVELASQUEZ - 01/08/2013: Se anadio la posibilidad de ocultar columnas, que las tablas se llenen de manera no asincronica,
un data-tipo para poder mostrar html en las columnas y que 1 y 0 tambien puedan interpretarse como  booleans para mayor
correspondencia con la base de datos.
JVELASQUEZ - 02/09/2013: Se anadieron los campos autonumerico para mostrar correlativos de la tabla y checkbox para
...
CJORELLANA - 15/09/2013: Se anadieron el boton de ver.
07/09/2013: Adaptar a Twitter Bootstrap 3.0
18/09/2013: Usar fechas UTC
02/10/2013: Agregar operadores logicos para buscar
*/
        ...
        else
        {
            ...
            switch (segmento)
            {
                ...
                case "dd":
                    ...
                    break;
                case "ddd":
                    segmento = Date.nombres.dias[this.getUTCDay()].substring(0, 3);

                    break;
                case "dddd":
                    segmento = Date.nombres.dias[this.getUTCDay()];

                    break;
                case "MM":
                    segmento = this.getUTCMonth() + 1;
                    ...
                    break;
                case "MMM":
                    segmento = Date.nombres.meses[this.getUTCMonth()].substring(0, 3);

                    break;
                case "MMMM":
                    segmento = Date.nombres.meses[this.getUTCMonth()];

                    break;
                case "yyyy":
                    segmento = this.getUTCFullYear();
...
Number.prototype.formato = function (Formato)
{
    ...
    if (IndiceDecimal > -1)
    {
        ...
        for (var i = IndiceDecimal; i < Formato.length; i++)
        {
            var digito = val.toString().charAt(Indice + (i - IndiceDecimal));
        ...
        {	
            ...
            if (Tipo == "AND")
            {
	           	$.each(Criterios, function(j, val)
		        {
		        	...
		        	if (val != "")
		        	{
						...
						{
			            	if (celda.toString().toUpperCase().indexOf(val) !== -1)
							{
								Encontrado = true;
								return false;
							}
            ...
            else
            {
	            $.each(this, function (i, celda)
	            {
	            	if (Tipo == "")
	            	{
						if (celda.toString().toUpperCase().indexOf(Criterios) !== -1)
						{
			            	Respuesta = true;
							return false;
						}
	            	...
	            	{	
		            	$.each(Criterios, function(j, val)
		            	{
			            	if (val != "" && celda.toString().toUpperCase().indexOf(val) !== -1)
							{
			                    Respuesta = true;
		                    	return false;
							}
            ...
            if (Campo == undefined)
            {
                ...
                else
                {
                    ...
                    switch (Boton) {
                    	...
                    	//cjorellana - 12/09/2013 - se agrega boton ver
						case "ver":
							var i = $(document.createElement('i'));
